...
export default async function handler(req: any, res: any) {
  ...
  if (type === "text" && !body.content?.trim()) {
    return res.status(400).json({ error: "Invalid payload: content is required for text analysis." });
  }
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents:
        type === "text"
          ? `Analyze this message text: "${body.content}"`
          : {
              parts: [
                {
                  inlineData: {
    ...
    if (!rawText) {
      console.error("Gemini response missing text", { type, model: MODEL_NAME });
    ...
    try {
      ...
      console.error("Gemini response was not valid JSON", { type, model: MODEL_NAME });
